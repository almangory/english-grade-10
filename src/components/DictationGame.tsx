...
export default function DictationGame({ 
  ...
  const handleCheckSpelling = () => {
    ...
    if (isAnswerCorrect) {
      ...
      // Random encouraging Arabic/English feedback
      const praises = [
        "Excellent! رائع جداً! ✨",
        "Amazing spelling! تهجئة ممتازة! 🎉",
        "Perfect spelling! أحسنت، إجابة نموذجية! 🌟",
        "Great job, you are a champion! عمل مذهل، أنت بطل! 💪",
        "Outstanding! ممتاز ومبدع! 🏆"
      ];
      ...
      setStreak(0);
      const errors = [
        "Not quite right! حاول مرة أخرى بتركيز! 💪",
        "Check your spelling and try again! أعد كتابتها بتركيز! ✍️",
        "Almost there! Try one more time! أوشكت على الوصول، جرب مجدداً!"
      ];
  ...
  return (
    <div className="bg-white p-6 sm:p-8 rounded-[36px] border-b-8 border-r-8 border-violet-100 shadow-sm flex flex-col gap-6 max-w-3xl mx-auto w-full text-right" dir="rtl">
      
      {/* Arabic/English Title Banner */}
      <div className="border-b-4 border-dashed border-violet-100 pb-5 text-center">
        ...
        <p className="text-xs font-bold text-slate-500 mt-2 leading-relaxed">
          استمع إلى الكلمات الإنجليزية الصوتية من كتاب الصف الأول الثانوي (SMILE Grade 10) واكتبها بطريقة صحيحة لكسب النجوم والجوائز! 🌟
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        {/* Unit Selector (Right/Left adaptive grid span 7) */}
        <div className="md:col-span-7 bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col gap-2">
          <label className="text-xs font-black text-slate-700 flex items-center gap-1">
            ...
            اختر وحدة التدريب (Unit Selection):
          </label>
          ...
          <p className="text-[10px] text-slate-400 font-bold mt-1">
            * سيتم تصفية وإسماع الكلمات الصوتية المقررة في المنهج للوحدة المختارة فقط.
          </p>
        </div>
        ...
        <div className="md:col-span-5 grid grid-cols-2 gap-2 bg-gradient-to-br from-violet-50/50 to-indigo-50/50 p-4 rounded-2xl border border-violet-100">
          ...
          <div className="bg-white p-3 rounded-xl border border-violet-100 text-center flex flex-col justify-center items-center">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase">مجموع النجوم</span>
            <span className="text-lg font-black text-violet-950">{score} 🌟</span>
          </div>

          <div className="bg-white p-3 rounded-xl border border-violet-100 text-center flex flex-col justify-center items-center">
            <Flame className="w-6 h-6 text-orange-500 mb-1" />
            <span className="text-[10px] font-black text-slate-500 uppercase">التتابع المتتالي</span>
          ...
          <div className="col-span-2 bg-white/80 py-2 px-4 rounded-xl border border-violet-100 text-center flex justify-between items-center text-xs">
            <span className="font-bold text-slate-500">🏆 أعلى مجموع إملاء:</span>
            <span className="font-black text-violet-950">{highScore} نجوم</span>
          </div>
      ...
      {activeWord ? (
        <div className="bg-violet-50/40 p-6 rounded-[28px] border-2 border-violet-100 flex flex-col items-center gap-6 relative overflow-hidden text-center">
          ...
          <div className="absolute top-3 right-4 bg-violet-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider">
            المستوى {activeWord.unit} • Unit {activeWord.unit}
          </div>
          ...
          <div className="flex flex-col items-center gap-3 mt-2">
            ...
            >
              ...
              <span className="text-[11px] font-black uppercase tracking-wider mt-1.5">انطق الكلمة</span>
          ...
          <div className="w-full max-w-md flex flex-col gap-2.5">
            <div className="relative">
              <input
                ...
                placeholder="اكتب الكلمة الإنجليزية هنا... (e.g. Science)"
                dir="ltr"
              ...
              />
              <Keyboard className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300 pointer-events-none" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-1">
              {!hasChecked ? (
                ...
                >
                  ...
                  <span>تحقق من الإجابة (Verify Spelling)</span>
                </button>
              ) : (
                <>
                  <button
                  ...
                  >
                    <span>الكلمة التالية (Next Word)</span>
          ...
          <AnimatePresence>
            {hasChecked && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className={`w-full max-w-md p-4 rounded-2xl border-2 flex flex-col gap-1.5 text-right ${
                  isCorrect
              ...
              >
                <div className="flex items-center gap-2 font-black text-sm">
                  ...
                  <span>{isCorrect ? "تهجئة صحيحة! Excellent!" : "توجد أخطاء في الكلمة!"}</span>
                </div>
                ...
                {!isCorrect && (
                  <div className="bg-white p-2.5 rounded-xl border border-rose-100 mt-2 text-center" dir="ltr">
                    <span className="text-[10px] block font-bold text-slate-400 uppercase tracking-widest">الكتابة الصحيحة:</span>
                    <span className="text-lg font-black text-rose-600 tracking-wider uppercase">{activeWord.word}</span>
                    <span className="text-xs block font-bold text-slate-500 mt-1">ترجمتها: {activeWord.arabic}</span>
                  </div>
                )}

                {isCorrect && (
                  <div className="bg-white p-2.5 rounded-xl border border-emerald-100 mt-2 text-center" dir="ltr">
                    <span className="text-[10px] block font-bold text-slate-400 uppercase tracking-widest">الترجمة والمعنى:</span>
          ...
          <div className="w-full border-t border-dashed border-violet-200 pt-5 flex flex-col gap-3">
            <span className="text-xs font-black text-violet-950 self-start">💡 وسائل المساعدة المتاحة (Hints Area):</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 w-full text-center">
              ...
              <div className="bg-white p-3 rounded-2xl border border-slate-200 flex flex-col justify-between items-center gap-2">
                <span className="text-[10px] font-black text-slate-400 uppercase">ترجمة الكلمة بالعربية</span>
                {showArabicHint ? (
              ...
              <div className="bg-white p-3 rounded-2xl border border-slate-200 flex flex-col justify-between items-center gap-2">
                <span className="text-[10px] font-black text-slate-400 uppercase">الجملة التوضيحية</span>
                {showExampleHint ? (
              ...
              <div className="bg-white p-3 rounded-2xl border border-slate-200 flex flex-col justify-between items-center gap-2">
                <span className="text-[10px] font-black text-slate-400 uppercase">كشف الأحرف المساعدة</span>
                <button
                ...
                >
                  كشف حرف مساعد ({revealedLettersCount}/{activeWord.word.length}) 🔍
                </button>
              </div>
        ...
        <div className="bg-slate-50 p-12 rounded-[32px] text-center border-2 border-dashed border-slate-200">
          ...
          <h4 className="text-lg font-black text-slate-700">لا تتوفر كلمات إملائية!</h4>
          <p className="text-xs font-bold text-slate-500 mt-1">برجاء التحقق من اختيار وحدات تحتوي على كلمات في المنهج.</p>
        </div>
      )}
      ...
      {history.length > 0 && (
        <div className="border-t border-dashed border-violet-150 pt-5 mt-2">
          <h4 className="text-sm font-black text-violet-950 mb-3 flex items-center gap-1.5 self-start">
            📊 الكلمات التي مارستها اليوم (Session History):
          </h4>
            ...
            {history.map((item, idx) => (
              ...
              >
                <div className="flex flex-col text-left">
                  ...
                  {!item.answeredCorrectly && (
                    <span className="text-[9px] text-rose-600 font-semibold line-through mt-0.5">محاولة: {item.userAttempt || "فارغ"}</span>
                  )}
                </div>
                <div className="shrink-0">
                  {item.answeredCorrectly ? (
                    <span className="bg-emerald-500 text-white text-[10px] font-black px-2 py-0.5 rounded-md flex items-center gap-0.5">
                      <Check className="w-3 h-3" /> صح
                    </span>
                  ) : (
